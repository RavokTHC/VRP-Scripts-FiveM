-- a basic desbugskins implementation

local cfg = module("cfg/desbugskins")
local lang = vRP.lang
local desbugskins = cfg.desbugskins
...
-- name = partid
function vRP.opendesbugskins(source,parts)
...
local function build_client_desbugskins(source)
  local user_id = vRP.getUserId(source)
  if user_id ~= nil then
    for k,v in pairs(desbugskins) do
      local parts,x,y,z = table.unpack(v)

      local desbugskins_enter = function(source)
        local user_id = vRP.getUserId(source)
        if user_id ~= nil then

          vRP.opendesbugskins(source,parts)
        end
      end

      local function desbugskins_leave(source)
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    build_client_desbugskins(source)
  end
end)
