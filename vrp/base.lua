          ...
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") [vrP] Sem Whitelist (seu id = "..user_id..")")

            for i = 1, 2 do
                deferrals.update("[vRP] Sem Whitelist (seu id = "..user_id..").")
                Wait(5000)
            end
                deferrals.done("[vRP] Sem Whitelist (seu id = "..user_id..").")
              end
            end)
          else
